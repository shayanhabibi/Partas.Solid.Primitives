#r "nuget: Partas.Fake.Tools.GitCliff,0.2.0"
...
type PackageName = private PackageName of string with
...
module PackageName =
    ...
    let trimVersion packageName = packageName |> toTagSuffix |> Str.delete
...
{% if version -%}
    ## [{{ version | trim_start_matches(pat="v") | {{{PackageName.
                                                              configVersionTrimmer
                                                          packageName}}} }}] - {{ timestamp | date(format="%Y-%m-%d") }}
{% else -%}
    <h2>
...
"""
    ...
    let private trimVersions packageName (context: JsonContent) =
       ...
       context
       |> List.map modifyContext : JsonContent
       ,lastVersion
    ...
    let private getModifiedContext packageName dir =
        ...
        fun ctx -> {
            ctx with GitCliff.CliParams.FromContext = Files.``gitcliff-context.json``
        }
    ...
    let bumpWithModifiedContext packageName initialVersion dir =
        ...
        let bumpedContextCliParams: GitCliff.CliParams -> _ = fun p -> {
            p with
        ...
        let newVersion =
            ...
            |> Option.flatten
        GitCliff.run (fun p ->
            { p with FromContext = Files.``gitcliff-context.json`` }) dir
        File.delete Files.``gitcliff-context.json``
        ...
        |> function
            | Some value ->
                value + PackageName.toTagSuffix packageName
                |> Git.Branches.tag dir
            | _ -> ()
