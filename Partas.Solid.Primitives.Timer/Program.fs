...
module private TimerSpec =
    ...
    module Spec =
        [<Literal>]
        let path = "@solid-primitives/timer"
...
type Timer =
    ...
    [<ImportMember(path)>]
    static member makeTimer(callback: unit -> unit, timespan: int, policy: IntervalOrTimeout) : DisposeCallback =
        jsNative
    ...
    static member createTimer
        (callback: unit -> unit, timespan: unit -> U2<bool, int>, policy: IntervalOrTimeout)
        : unit =
        jsNative

    /// makeTimer but with a fully reactive delay. The delay can also be set to 'false' in which case the timer is disabled.
    [<ImportMember(path)>]
    static member createTimer(callback: unit -> unit, timespan: unit -> int, policy: IntervalOrTimeout) : unit =
        jsNative

    /// makeTimer but with a fully reactive delay. The delay can also be set to 'false' in which case the timer is disabled.
    [<ImportMember(path)>]
    static member createTimer(callback: unit -> unit, timespan: unit -> bool, policy: IntervalOrTimeout) : unit =
        jsNative
